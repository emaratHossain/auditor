...
test('paste a URL, fill in the numbers, read the report', async ({ page }) => {
  ...
  await expect(page.getByText('out of 100')).toBeVisible({ timeout: 30_000 })
  ...
  await page.getByRole('button', { name: /How was this number built/i }).click()
  await expect(page.getByText('Main button effectiveness')).toBeVisible()
  await expect(page.getByText(/not a real accessibility audit/i)).toBeVisible()
})
