...
**The cascade runs inside the database**, so Eloquent's `deleting` events never
fire for the audits. Collecting the ids before the delete is what makes file
cleanup possible at all; a model observer would not have worked. (Written
against SQLite; the move to MySQL on 2026-08-06 changes nothing here — an InnoDB
`ON DELETE CASCADE` is equally invisible to Eloquent.)
