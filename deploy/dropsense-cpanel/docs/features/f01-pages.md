...
flowchart LR
    ...
    B --> C["Delete the page in a transaction<br/><i>the database cascades to every child row</i>"]
...
**Why the ids are collected before the delete.** The cascade runs inside the database, so
Eloquent's `deleting` events never fire for the audits. An observer on `Audit` would simply
never run, and every screenshot would be orphaned. This was written against SQLite and is
unchanged by the move to MySQL — an InnoDB `ON DELETE CASCADE` is just as invisible to
Eloquent, so the reason the ids are collected up front still stands.
