...
Laravel 12 · PHP 8.3 · **MySQL** · **database queue driver** · React 19 + Vite +
...
**The database was SQLite and is now MySQL** (changed 2026-08-06, on request). Anything
dated earlier — the two design specs, `docs/superpowers/plans/`, older feature docs — says
SQLite because that is what was true when it was written. This line is the current answer.
Nothing else about the stack moved: the queue, cache and sessions still ride on database
tables, so there is still no Redis and no Horizon.

Databases: `auditor` for development, `auditor_test` for the suite. Create them with
`mysql -u root -e "CREATE DATABASE auditor"` before the first `php artisan migrate`.
